class GtaScm::Panel::ThreadInfo2 < GtaScm::Panel::Base
  def initialize(*)
    super
    ty = 0
    ...
    3.times do |i|
      ...
      self.elements[:"right_label_#{i}"] = RuTui::Text.new(x: dx(tx+txxx), y: dy(ty), text: "")
      self.elements[:"right_text_#{i}"] = RuTui::Text.new(x: dx(tx+txx+txxx), y: dy(ty), text: "")
      ty += 1
    end
    self.elements[:left_label_0].set_text("ID:")
    self.elements[:left_label_1].set_text("Name:")
    self.elements[:left_label_2].set_text("Type:")
    # self.elements[:left_label_3].set_text("")
    self.elements[:right_label_0].set_text("Wake:")
    self.elements[:right_label_1].set_text("BasePC:")
    self.elements[:right_label_2].set_text("PC:")
  ...
  def update(process,is_attached,focused = false)
    ...
    if thread = process.cached_threads[self.settings[:thread_id]]
      ...
      self.elements[:left_text_2].set_text("#{thread.status}")
      # self.elements[:left_text_3].set_text("")
      self.elements[:right_text_0].set_text("#{thread.wake_time}")
      self.elements[:right_text_1].set_text("#{thread.base_pc_scm}")
      self.elements[:right_text_2].set_text("#{thread.scm_pc}")
